...
- **Vitrine pública**: `/vitrine/:userId` no frontend, alimentada por `GET /api/v1/public/storefront/{user_id}` — só mostra campanhas com `affiliate_link` preenchido e status `posted`/`scheduled`. Cada card mostra a **miniatura do produto** (`image_url`, com fallback pro emoji 🛍️ e `onError` que esconde imagem quebrada) + seta CTA + tag "MAIS VENDIDO" nas que já venderam. **Privacidade**: se o usuário não definiu um nome de vitrine, cai em "Minha Vitrine" — **nunca expõe o `full_name`** publicamente. Editável em "Sua vitrine pública" no dashboard
- **Integração Instagram** (Graph API — Content Publishing): `services/instagram_service.py` (fluxo oficial de 2 passos: `POST /{ig_user_id}/media` → `POST /{ig_user_id}/media_publish`) + `routers/instagram.py` (`/api/v1/instagram/connect`, `/status`, `/campaigns/{id}/publish`) + tool MCP `postar_no_instagram`. Usa `graph.facebook.com` (host correto pra publicação; **não** `graph.instagram.com`, que é a Basic Display e não publica) com o **IG Business Account ID** vinculado a uma Página do Facebook. Token de longa duração por usuário no model `User` (`instagram_access_token`/`instagram_user_id`/`instagram_token_expires_at`). Feed exige mídia → `Campaign.image_url` (migration **0009**; a 0008 do main adiciona os campos `instagram_*` em `users`; campo `image_url` opcional no form de nova campanha e mostrado na vitrine). Envs `INSTAGRAM_APP_ID`/`INSTAGRAM_APP_SECRET` (opcionais pro fluxo atual — o `/connect` só valida o token colado). Conta Business vinculada a uma Página do FB. **Publicar com a própria conta funciona em modo Desenvolvimento, sem App Review**; a App Review (`instagram_content_publish`+`instagram_basic`) só é necessária pra liberar a outros usuários. Ver a seção "Integração Instagram" abaixo pras pegadinhas do setup no Meta. Só API oficial, sem automação de navegador. **Frontend**: `hooks/useInstagram.js` + `components/InstagramSection.jsx` (seção no dashboard pra conectar conta colando token + IG Business Account ID; mostra status) e botão "Postar no Instagram" no `CampaignCard` (aparece quando a conta está conectada e a campanha tem `image_url`; mostra "✓ Publicado" com link do post). Tudo pronto, mas o publish só funciona de fato após a App Review da Meta.
